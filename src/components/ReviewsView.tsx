      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
          {criticalReviews.map((rev) => (
            <div
              key={rev.id}
              className="bg-white border border-stone-200 p-8 sm:p-10 rounded-xl hover:border-gold-300 transition-all flex flex-col justify-between shadow-xs hover:shadow-md relative overflow-hidden group"
            >
              {/* Background decorative quote watermark */}
              <Quote
                size={72}
                className="absolute -right-2 -bottom-2 text-stone-100 pointer-events-none group-hover:text-gold-50 transition-colors"
              />

              <div className="relative z-10">
                <Quote size={28} className="text-gold-600 mb-6" />
              ...
              <div className="pt-6 mt-8 border-t border-stone-200/80 relative z-10">
                <p className="font-sans font-bold text-sm tracking-wide text-navy-900">
                  — {rev.publication}
                </p>
                {rev.critic && (
