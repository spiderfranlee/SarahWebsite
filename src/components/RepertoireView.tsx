    ...
    >
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-5 py-2.5 bg-navy-900 hover:bg-navy-850 text-gold-300 text-xs font-sans font-bold tracking-wider uppercase rounded-md shadow-sm transition-colors cursor-pointer"
            >
        ...
        <div className="flex flex-wrap items-center gap-2 mb-8 border-b border-stone-200 pb-4">
          ...
          >
            <BookOpen size={14} className={activeTab === "opera" ? "text-gold-400" : "text-stone-500"} />
          ...
          <button
            ...
            className={`px-4 py-2.5 text-xs font-sans tracking-wider uppercase font-bold rounded-md transition-all cursor-pointer flex items-center gap-2 ${
              activeTab === "ceremony"
                ...
                : "bg-white text-navy-900 hover:bg-gold-50 border border-stone-200"
            }`}
          >
            <Heart size={14} className={activeTab === "ceremony" ? "text-gold-400" : "text-gold-600"} />
            <span>Ceremony Music Guide</span>
            <span
              className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${
                activeTab === "ceremony" ? "bg-navy-800 text-gold-200" : "bg-gold-100 text-gold-800"
              }`}
            >
              Weddings & Services
            </span>
          ...
          >
            <Music size={14} className={activeTab === "concert" ? "text-gold-400" : "text-stone-500"} />
          ...
          >
            <Sparkles size={14} className={activeTab === "song" ? "text-gold-400" : "text-stone-500"} />
        ...
        <div className="mb-10 flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-10 py-3 bg-white border border-stone-300 rounded-md text-sm font-sans text-stone-900 placeholder-stone-400 focus:outline-none focus:border-gold-500 focus:ring-1 focus:ring-gold-500 shadow-xs transition-colors"
            />
            {searchQuery && (
              <button
          ...
          {activeTab === "opera" && (
            <div className="flex flex-wrap items-center gap-2">
              <select
                ...
                className="px-3 py-2.5 bg-white border border-stone-300 rounded-md text-xs font-sans font-semibold text-stone-700 focus:outline-none focus:border-gold-500 shadow-xs cursor-pointer"
              >
        ...
        {activeTab === "ceremony" && (
          <div className="space-y-10 animate-fadeIn">
            {/* Introductory Banner */}
            <div className="bg-gradient-to-br from-gold-50/80 via-white to-stone-50 border border-gold-200/80 rounded-xl p-6 sm:p-8 shadow-xs">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                  <div className="inline-flex items-center gap-2 px-3 py-1 bg-white border border-gold-200 rounded-full text-[11px] font-sans font-bold tracking-widest text-navy-950 uppercase mb-3 shadow-2xs">
              ...
              <div className="mt-6 pt-6 border-t border-stone-200/80 flex flex-wrap gap-2 items-center">
                ...
                {ceremonyMusicGuide.sections.map((sec) => (
                  <a
                    key={sec.id}
                    href={`#${sec.id}`}
                    className="text-xs font-sans font-semibold px-3 py-1 bg-white hover:bg-gold-50 text-stone-700 hover:text-navy-950 border border-stone-200 hover:border-gold-300 rounded-full transition-colors cursor-pointer shadow-2xs"
                  >
                    {sec.stage}
                  </a>
                ))}
            ...
            {filteredCeremonySections.length === 0 ? (
              <div className="p-12 text-center bg-white rounded-xl border border-stone-200">
                ...
                <button
                  onClick={() => setSearchQuery("")}
                  className="mt-3 text-xs font-sans font-bold text-navy-900 hover:text-gold-700 underline uppercase tracking-wider cursor-pointer"
                >
                  Clear search
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {filteredCeremonySections.map((section) => (
                  <div
                    key={section.id}
                    id={section.id}
                    className="bg-white border border-stone-200 rounded-xl overflow-hidden shadow-xs hover:border-gold-300 hover:shadow-md transition-all flex flex-col"
                  >
                    {/* Card Header */}
                    ...
                    <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
                        ...
                        <ul className="space-y-2.5">
                          {section.pieces.map((piece, pIdx) => {
                            ...
                            return (
                              ...
                              >
                                <span className="mt-1 w-2 h-2 rounded-full bg-gold-500 shrink-0 group-hover/item:scale-125 transition-transform" />
                      ...
                      {section.note && (
                        <div className="p-3.5 bg-gold-50/70 border border-gold-200/80 rounded-lg text-xs font-sans text-stone-700 leading-relaxed flex items-start gap-2.5">
                          <Info size={15} className="text-gold-700 shrink-0 mt-0.5" />
            ...
            <div className="bg-gold-50/60 border border-gold-200/80 rounded-xl p-6 sm:p-8 shadow-xs">
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                <div className="space-y-4 max-w-3xl">
                  <div className="inline-flex items-center gap-2 text-xs font-sans tracking-widest text-gold-800 font-bold uppercase">
                    <Heart size={14} className="text-gold-700" />
                  ...
                  <ul className="space-y-2.5 pt-1">
                    {ceremonyMusicGuide.notesForCouples.map((note, idx) => (
                      <li key={idx} className="flex items-start gap-3 text-sm font-sans text-stone-700">
                        <CheckCircle2 size={16} className="text-gold-700 shrink-0 mt-0.5" />
              ...
              Object.keys(operaByComposer).map((composer) => {
                ...
                return (
                  ...
                  <div className="flex items-center justify-between pb-4 mb-6 border-b border-stone-200">
                    <div>
                      <span className="text-[11px] font-sans tracking-[0.25em] text-gold-700 font-bold uppercase block">
                        Composer
                      </span>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {roles.map((role) => (
                      <div
                        key={role.id}
                        className="p-5 bg-stone-50/70 hover:bg-white border border-stone-200 hover:border-gold-300 rounded-lg transition-all shadow-2xs hover:shadow-sm flex flex-col justify-between gap-3"
                      >
                        <div>
                            ...
                            <span
                              className={`text-[10px] tracking-wider uppercase font-bold px-2.5 py-0.5 rounded-full shrink-0 border ${
                                role.status === "Performed"
                                  ...
                                  ? "bg-gold-50 text-navy-950 border-gold-300"
                                  : role.status === "Covered"
                          ...
                          {role.company && (
                            <div className="mt-2 text-[11px] font-sans text-navy-950 bg-gold-50/80 px-2.5 py-1 rounded border border-gold-200">
            ...
            {filteredConcert.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {filteredConcert.map((item) => (
                  <div
                    key={item.id}
                    className="bg-white border border-stone-200 p-6 rounded-xl shadow-xs hover:border-gold-300 hover:shadow-md transition-all flex flex-col justify-between"
                  >
                    <div>
                      <span className="text-[11px] font-sans tracking-widest text-gold-700 font-bold uppercase block mb-1">
                        {item.composer}
                      </span>
            ...
            {filteredSongCycles.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {filteredSongCycles.map((sc) => (
                  <div
                    key={sc.id}
                    className="bg-white border border-stone-200 p-6 sm:p-8 rounded-xl shadow-xs hover:border-gold-300 hover:shadow-md transition-all flex flex-col justify-between"
                  >
                    <div>
                      <span className="text-[11px] font-sans tracking-widest text-gold-700 font-bold uppercase block mb-1">
                        {sc.composer}
